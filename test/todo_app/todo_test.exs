defmodule TodoApp.TodoTest do
  ...
  describe "tasks" do
    ...
    test "create_task/1 with valid data creates a task" do
      valid_attrs = %{assigned_user: 42, description: "some description", priority: "some priority", title: "some title"}

      assert {:ok, %Task{} = task} = Todo.create_task(valid_attrs)
      assert task.assigned_user == 42
    ...
    test "update_task/2 with valid data updates the task" do
      task = task_fixture()
      update_attrs = %{assigned_user: 43, description: "some updated description", priority: "some updated priority", title: "some updated title"}

      assert {:ok, %Task{} = task} = Todo.update_task(task, update_attrs)
      assert task.assigned_user == 43
